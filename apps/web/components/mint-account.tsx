...
export function MintAccount() {
  ...
  const verify = useCallback(async () => {
    setStatus("checking");
    try {
      const account = await connection.getAccountInfo(APPROVED_PWRC_MINT, "finalized");
      ...
      const mint = await getMint(connection, APPROVED_PWRC_MINT, "finalized", account.owner);
      if (mint.decimals !== PWRC_DECIMALS) throw new Error(`Unexpected mint decimals: ${mint.decimals}.`);
      if (mint.supply > PWRC_GENESIS_SUPPLY_BASE_UNITS) throw new Error("Mint supply exceeds the frozen PTK-001 maximum.");
      if (mint.mintAuthority !== null) throw new Error("Mint authority has not been revoked.");
      if (mint.freezeAuthority !== null) throw new Error("Freeze authority has not been revoked.");
      setStatus("verified");
      setDetail(`Verified at finalized commitment · Supply ${mint.supply.toString()} base units`);
    } catch (error) {
      setStatus("error");
